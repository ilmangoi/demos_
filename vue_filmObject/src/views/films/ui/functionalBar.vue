...
const { mapActions, mapMutations, mapState } = createNamespacedHelpers('films')

export default {
  methods: {
    ...mapMutations(['setAddDialogVisibility', 'changeEditableState', 'changeSelectedItem']),
    ...mapActions(['getFilmListAction', 'deleteFilmAction', 'getFilmTotalAction']),
    searchFilmHandler: _.throttle((keyword, that) => {
      keyword = keyword.replace(/\s+/g, '') // 把关键字中的空格过滤掉
      that.getFilmListAction({ keyword }) // 请求电影列表(关键字查询到的)
    }, 500),
    handleDelete() {
      ...
      Notify.confirm('delete', `确定全部删除吗？`, async (action, instance, done) => {
        if (action === 'confirm') {
          ...
          await this.deleteFilmAction({ id: this.selectedItemList })
