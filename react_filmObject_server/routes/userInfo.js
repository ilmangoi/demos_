...
router.post("/addUserinfo", uploader, async function (req, res, next) {
  ...
  if (req.files.length) {
    req.body.avatar = `http://localhost:8888/public/images/avatar/${req.files[0].filename}`;
  } else {
    if (req.body.sex === 1) {
      req.body.avatar = `http://localhost:8888/public/images/avatar/male_default.webp`;
    } else if (req.body.sex === 2) {
      req.body.avatar = `http://localhost:8888/public/images/avatar/female_default.webp`;
    }
  }
