...
// 把chunk字段的数据当做文件来接收
router.post("/uploadChunkFile", videoUploader.single("chunk"), (req, res) => {
...
const imgStorage = multer.diskStorage({
  destination: function (req, file, cb) {
    const dir = req.query.dir || "common";
...
router.post("/uploadImg", imgUpload.single("file"), (req, res) => {
  const dir = req.query.dir || "common";
  res.send({
    code: 0,
    msg: "ok",
    data: {
      path: `http://localhost:8888/public/images/${dir}/${req.file.filename}`,
    },
  });
});
